...
public abstract class OperatorChain<OUT, OP extends StreamOperator<OUT>>
    ...
    public OperatorChain(
        ...
        LOG.info(">>> taskname {} , operator factory is {}",
                containingTask.getName(), operatorFactory.getClass().getName());
        ...
        try {
            ...
            for (RecordWriterOutput<?> output : this.streamOutputs) {
                LOG.info(">>>> TaskName {} after createChainOutputs , output is {} ",
                        containingTask.getName(), output.toString());
            }
    ...
    private void createChainOutputs(
        ...
        for (int i = 0; i < outputsInOrder.size(); ++i) {
            ...
            //LOG.info(">>>> createChainOutputs task {} the  index i: {} , sourceNodeId {},   outputs tag {}, output toString {}",
            //        containingTask.getName(), i, output.getOutputTag(), output.getSourceNodeId() ,output.toString() );
    ...
    private <T> WatermarkGaugeExposingOutput<StreamRecord<T>> createOutputCollector(
        ...
        Integer currentOperatorVID = operatorConfig.getVertexID();
        OperatorID currentOpID = operatorConfig.getOperatorID();
        String currentOperatorName = operatorConfig.getOperatorName();
        String currentTaskName = containingTask.getName();

        LOG.info(">>>> createOutputCollector beginning taskName {}, process op VID  {}, opeatorID {}, opeatorName {} ",
                currentTaskName, currentOperatorVID, currentOpID, currentOperatorName);
        ...
        for (NonChainedOutput streamOutput :
            ...
            LOG.info(">>>> createOutputCollector getOperatorNonChainedOutputs taskName {}, process op VID  {}, opeatorID {}, opeatorName {} ",
                    currentTaskName, currentOperatorVID, currentOpID, currentOperatorName);
            LOG.info(">>>>>createOutputCollector getOperatorNonChainedOutputs taskName {}, nonChainedOutput streamOutput tag {},  toString{}, sourceNode id {}, datasetID {}" ,
                    containingTask.getName(), streamOutput.getOutputTag(), streamOutput.toString(),
                    streamOutput.getSourceNodeId(), streamOutput.getDataSetId());
        ...
        List<StreamEdge> outputEdges = operatorConfig.getChainedOutputs(userCodeClassloader);
        for (StreamEdge outputEdge : operatorConfig.getChainedOutputs(userCodeClassloader)) {
            int outputId = outputEdge.getTargetId();
            StreamConfig chainedOpConfig = chainedConfigs.get(outputId);

            Integer outputVID = chainedOpConfig.getVertexID();
            OperatorID outputOpID = chainedOpConfig.getOperatorID();
            String  outputOperatorName = chainedOpConfig.getOperatorName();

            LOG.info(">>>> getChainedOutputs taskName {}, process op VID  {}, operatorID {}, operatorName {}, output ID {}, output VID  {} output opID {}, output Opname {} ",
                    currentTaskName, currentOperatorVID, currentOpID, currentOperatorName, outputId, outputVID, outputOpID, outputOperatorName);
